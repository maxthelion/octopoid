...
def process_actions(ctx: JobContext) -> None:
    ...
    maps to a handler registered in orchestrator/actions.py via @register_action.
    ...
    for action in actions:
        ...
        _debug_log(f"process_actions: dispatching {action_type} (id={action_id})")
        ...
        if handler is None:
            ...
            try:
                sdk.actions.fail(action_id, error_msg)
            except Exception as e:
        ...
        try:
            ...
            _debug_log(f"process_actions: {action_type} (id={action_id}) succeeded")
            try:
