...
_INBOX_WORKER_PROMPT_TEMPLATE = """\
You are a worker agent executing a command on behalf of a user action.
...
Execute the command above. You have access to:
- **Bash** — run Python scripts, CLI tools
- **Read / Write / Edit / Glob / Grep** — inspect and edit files

You are running in the octopoid project root: `{parent_project}`

The octopoid orchestrator package is available for import. To use the SDK:
...
After executing the command, post a result message back using the SDK:
...
sdk.messages.create(
    ...
    to_actor='human',
    type='worker_result',
    content='Completed: <brief summary of what was done>',
)
```

If the task_id is empty or not applicable, use 'worker' as task_id.

## If You Cannot Complete the Command
...
Do not write any files outside the project. Do not make git commits or push branches.
"""


def _load_inbox_state(state_file: Path) -> dict:
    """Load inbox processor state from disk."""
    if not state_file.exists():
    ...
    try:
        ...
        _debug_log(f"process_inbox: could not load state ({e}), starting fresh")
        return {"processed_message_ids": []}


def _save_inbox_state(state_file: Path, state: dict) -> None:
    """Persist inbox processor state to disk."""
    try:
        state_file.parent.mkdir(parents=True, exist_ok=True)
        with open(state_file, "w") as f:
            json.dump(state, f, indent=2)
    except OSError as e:
        _debug_log(f"process_inbox: could not save state: {e}")


def _spawn_inbox_worker(
    message_id: str,
    prompt: str,
    parent_project: Path,
    orchestrator_pythonpath: Path,
) -> int | None:
    """Spawn a lightweight worker agent to process an inbox message.

    Creates a worker directory, writes the prompt, and invokes claude -p.
    The worker runs in the parent project directory (no worktree needed).

    Args:
        message_id: Message ID (used for directory naming)
        prompt: The full prompt text for the worker
        parent_project: Path to the parent project root (worker's cwd)
        orchestrator_pythonpath: Path to the orchestrator package directory

    Returns:
        PID of the spawned process, or None if spawn failed
    """
    from .config import get_runtime_dir

    workers_dir = get_runtime_dir() / "inbox_workers"
    workers_dir.mkdir(parents=True, exist_ok=True)

    worker_dir = workers_dir / message_id
    ...
    env["ORCHESTRATOR_PYTHONPATH"] = str(orchestrator_pythonpath)
    ...
    try:
        ...
        _debug_log(f"process_inbox: spawned worker for message {message_id} with PID {process.pid}")
        return process.pid
    except Exception as e:
        _debug_log(f"process_inbox: failed to spawn worker for message {message_id}: {e}")
        return None


@register_job
def process_inbox(ctx: JobContext) -> None:
    """Poll for unprocessed inbox messages and spawn worker agents.

    Fetches messages where to_actor='worker' that haven't been processed yet.
    For each new message, spawns a lightweight Claude worker agent with the
    message content as instructions.

    Processed message IDs are persisted in
    .octopoid/runtime/inbox_processor_state.json so messages are not
    re-processed across scheduler ticks.
    """
    from .sdk import get_sdk
    from .config import get_orchestrator_dir, find_parent_project

    state_file = get_orchestrator_dir() / "runtime" / "inbox_processor_state.json"
    parent_project = find_parent_project()
    orchestrator_pythonpath = parent_project / "orchestrator"

    state = _load_inbox_state(state_file)
    processed_ids: set[str] = set(state.get("processed_message_ids", []))

    sdk = get_sdk()

    try:
        messages = sdk.messages.list(to_actor="worker")
    except Exception as e:
        _debug_log(f"process_inbox: failed to fetch messages: {e}")
        return
    ...
    for message in messages:
        ...
        task_id = message.get("task_id") or "worker"
        content = message.get("content", "")

        if not content:
            _debug_log(f"process_inbox: message {message_id} has no content, skipping")
            processed_ids.add(message_id)
            continue

        prompt = _INBOX_WORKER_PROMPT_TEMPLATE.format(
            command=content,
            message_id=message_id,
            message_type=message_type,
            task_id=task_id,
            parent_project=str(parent_project),
            orchestrator_pythonpath=str(orchestrator_pythonpath),
        )

        pid = _spawn_inbox_worker(
            message_id=message_id,
            prompt=prompt,
            parent_project=parent_project,
            orchestrator_pythonpath=orchestrator_pythonpath,
        )

        # Mark as processed regardless of whether spawn succeeded.
        # This prevents infinite retry loops on messages that fail to spawn.
        processed_ids.add(message_id)
        if pid is not None:
            new_count += 1
            _debug_log(
                f"process_inbox: spawned worker (PID={pid}) for message {message_id} "
                f"(type={message_type})"
            )

    state["processed_message_ids"] = sorted(processed_ids)
    _save_inbox_state(state_file, state)

    if new_count:
        _debug_log(f"process_inbox: spawned {new_count} worker(s)")
