...
class ImplementerRole(BaseRole):
    ...
    def _generate_execution_notes(self, commits_made: int, turns_used: int, pr_url: str | None = None, stdout: str = "") -> str:
        """Generate a concise execution summary for the task.

        Args:
            commits_made: Number of commits created
            turns_used: Number of Claude turns used
            pr_url: PR URL if created
            stdout: Claude's output (last portion used for summary)
        ...
        # Basic stats
        if commits_made > 0:
            parts.append(f"Created {commits_made} commit{'s' if commits_made != 1 else ''}")
        else:
        ...
        # PR status
        if pr_url:
            parts.append("PR created")

        # Try to extract key information from stdout
        # Look for summary lines or key phrases
        summary_lines = []
        if stdout:
            # Get last 500 chars for summary extraction
            tail = stdout[-500:] if len(stdout) > 500 else stdout
            lines = tail.split('\n')

            # Look for sentences that sound like summaries
            for line in lines:
                line = line.strip()
                if any(keyword in line.lower() for keyword in ['implemented', 'added', 'updated', 'fixed', 'created', 'modified', 'ready', 'complete']):
                    if len(line) > 20 and len(line) < 120:  # Reasonable summary length
                        summary_lines.append(line)
                        if len(summary_lines) >= 2:  # Max 2 summary lines
                            break
        ...
        # Truncate if too long (keep under 500 chars for database field)
        if len(notes) > 450:
            notes = notes[:450] + "..."
    ...
    def _handle_implementation_result(
        ...
        if skip_pr:
            ...
            try:
                ...
                # Generate execution notes for direct merge
                # After merge, we're on base_branch, so count since branch_name's merge-base
                commits_count = get_commit_count(self.worktree)  # Count commits on current branch
                ...
                complete_task(
                    task_path,
                    f"Merged directly to {base_branch}",
...
"""
            ...
            # Generate execution notes for PR creation
            # Count commits on the feature branch
            commits_count = get_commit_count(self.worktree, branch=branch_name)
            turns_used = self.read_tool_count()
            execution_notes = self._generate_execution_notes(
                commits_made=commits_count,
                turns_used=turns_used,
            ...
            if has_uncommitted:
                ...
                # PR failed but no work to preserve - mark complete with error note
                turns_used = self.read_tool_count()
                execution_notes = f"Implementation attempted but PR creation failed. Used {turns_used} turns."
                complete_task(
                    task_path,
                    f"Implementation attempted but PR creation failed: {e}",
                    execution_notes=execution_notes
                )
...
"""
            ...
            if is_db_enabled():
                ...
                complete_task(task_path, result_msg, execution_notes=execution_notes)
