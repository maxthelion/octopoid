...
class ImplementerRole(BaseRole):
    ...
    def _generate_execution_summary(
        ...
        """Generate a concise execution summary for execution_notes field.

        Args:
            commits_count: Number of commits made
            turns_used: Number of Claude turns used
            pr_url: Optional PR URL if created
            stdout: Claude's stdout to extract key info

        Returns:
            Concise summary string (1-3 sentences)
        """
        # Extract last few lines of stdout for summary (skip verbose logs)
        summary_lines = []
        if stdout:
            # Get last 500 chars as they often contain the final summary
            tail = stdout[-500:].strip()
        ...
        if summary_lines:
            # Add agent's own summary if available
            parts.append(" ".join(summary_lines[:2]))

        if pr_url:
            parts.append("PR created and ready for review.")
        else:
            parts.append("Implementation complete.")
