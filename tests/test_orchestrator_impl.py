...
class TestTryMergeSubmodule:
    ...
    def test_push_failure_reverts_and_returns_false(self):
        """If push to origin fails, revert local merge and return False."""
        with patch.dict('os.environ', _ENV):
            role = self._make_role()
            sub_path = Path('/fake/agents/test-orch/worktree/orchestrator')
            main_checkout_sub = Path('/fake/project/orchestrator')
            venv_python = Path('/fake/venv/bin/python')
    ...
    def test_push_success_returns_true(self):
        ...
        with patch.dict('os.environ', _ENV):
            ...
            assert ['git', 'reset', '--hard', 'origin/main'] not in cmds
...
class TestTryMergeToMain:
    ...
    def test_submodule_ref_push_failure_returns_false(self):
        """If submodule ref push to main repo fails, return False."""
        with patch.dict('os.environ', _ENV):
            role = self._make_role()
            sub_path = Path('/fake/agents/test-orch/worktree/orchestrator')
            role.parent_project = Path('/fake/project')
            ...
            def mock_run_cmd(cmd, cwd, timeout=120):
                calls.append((cmd, str(cwd)))
                # Make git diff --cached --quiet return non-zero (indicating there ARE changes)
                if cmd == ['git', 'diff', '--cached', '--quiet']:
                    return _make_completed_result(1)
                # Fail the push to origin for main repo
                if cmd == ['git', 'push', 'origin', 'main'] and str(cwd) == '/fake/project':
                    return _make_completed_result(1, stderr='push rejected')
                return _make_completed_result(0)
            ...
            # Should fail because submodule ref push failed
            assert result is False

            # Verify the push was attempted
            cmds = [c[0] for c in calls]
            assert ['git', 'push', 'origin', 'main'] in cmds
