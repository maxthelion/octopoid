...
class TestCreateTaskProjectBranchInheritance:
    ...
    def test_task_inherits_project_branch(
        ...
        # Content must be sent to server
        assert "content" in call_kwargs, "create_task() must send content to server"
        assert project_branch in call_kwargs["content"], (
            "Content must include the project branch"
        )
    ...
    def test_explicit_branch_overrides_project_branch(
        ...
        assert explicit_branch in call_kwargs["content"], (
            "Content must use explicit branch"
        )
        assert project_branch not in call_kwargs["content"], (
            "Content must NOT use project branch when explicit branch is provided"
        )
