#!/usr/bin/env bash
# Guard: exit 1 (block spawn) when coverage-analyst drafts with status=idea or in_progress exist.
...
try:
    ...
    for status in ('idea', 'in_progress'):
        if sdk.drafts.list(status=status, author='coverage-analyst'):
            print('Pending coverage-analyst draft found — blocking spawn', file=sys.stderr)
            sys.exit(1)
    sys.exit(0)
