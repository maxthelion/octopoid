#!/usr/bin/env bash
# Guard: check for existing design-patterns-analyst drafts with status=idea or in_progress.
...
try:
    ...
    for status in ('idea', 'in_progress'):
        if sdk.drafts.list(status=status, author='design-patterns-analyst'):
            print('SKIP')
            break
